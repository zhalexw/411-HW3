...
def create_meal(meal: str, cuisine: str, price: float, difficulty: str) -> None:
    ...
    Returns: 
        None
    Raises: 
        1.Value error if price is negative 2.Value error if dificulty is not LOW, MED, or HIGH 3.Value error if there is a duplicate meal name 
    Logs: 
        error if there is a duplicate meal name
        """
...
def delete_meal(meal_id: int) -> None:
    ...
    Returns: 
        None 
    Raises: 
        1.Value Error if meal with meal id has already been deleted from database 2.Value error if meal with meal id not found 3. exception e if there is a database error
    Logs: 
        error if there is database error
    """
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
...
def get_meal_by_id(meal_id: int) -> Meal:
    ...
    Gets meal by meal id from database
     ...
     Raises: 
        Value error if meal with meal id has been deleted 2. Value error if Meal with Meal ID hasnt been found 3.Exception e if there is a database error

     Logs: 
        Error if there has been a database error 
...
def get_meal_by_name(meal_name: str) -> Meal:
    """Gets meal by meal name from database
     ...
     Returns: 
        Meal 
    Raises:
        Value error if meal with meal name has been deleted 2. Value error if Meal with Meal name hasnt been found 3.Exception e if there is a database error
     Logs:  
        Error if there has been a database error 
    """
    try:
...
def update_meal_stats(meal_id: int, result: str) -> None:
    ...
    Returns: 
        None 
    Raises: 
        Value error if meal with meal ID has been deleted 2. Value error if Meal with Meal ID hasnt been found 3.Exception e if there is a database error 4. Value Error if result is not win or loss
    Logs: 
        Error if there has been a database error 
        """
    try:
